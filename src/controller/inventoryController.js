...
exports.store =  async (req, res) =>{
    try {
      ...
      const {name_ingredient, user_id_user, ingredients_pic, buy_date, stock, unit, place, expiry_date } = req.body;
      
      // Cari ingredient berdasarkan nama
      const ingredient = await Ingredient.findOne({ where: { ingredient_name: name_ingredient } });
      ...
      ingredient = await Ingredient.findOne({ where: { ingredient_name: name_ingredient } });
...
exports.updateManual = async (req, res) => {
    try {
      ...
      if (updatedRows === 0) {
        return res.status(404).json({ message: "Inventory not found or no changes made" });
      }
...
exports.searchInventoryByIngredient = async (req, res) => {
  try {
    ...
    if (inventories.length === 0) {
      return res.status(404).json({ message: "No inventory items found matching the ingredient name." });
    }
